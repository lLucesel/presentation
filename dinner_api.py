...
식단짜기 : 6
종료하기 : 7\n"""
        self.return_menu = "반찬 / 국 고르기 >> 3 / 4    메뉴로 >> 5    식단짜기 >> 6   종료하기 >> 7"
...
async def read_root(request: Request):
    return templates.TemplateResponse("dinner_main_test.html", {"request": request})


@app.get("/select_menu/{menu_type}")
async def select_menu(menu_type: str, request: Request):
    if menu_type == "side_dish":
        menu_selector.print_side_dish()
    elif menu_type == "soup":
        menu_selector.print_soup()
    elif menu_type == "full_menu":
        menu_selector.print_menu()
    elif menu_type == "create_diet":
